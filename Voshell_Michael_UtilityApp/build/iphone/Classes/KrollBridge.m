...
NSString * Voshell_Michael_UtilityApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation Voshell_Michael_UtilityAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_voshell_michael_utilityapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_voshell_michael_utilityapp gc];
}
...
{
	// create Voshell_Michael_UtilityApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Voshell_Michael_UtilityApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_voshell_michael_utilityapp = [[Voshell_Michael_UtilityAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_voshell_michael_utilityapp];
	
	NSString *_voshell_michael_utilityappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _voshell_michael_utilityappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_voshell_michael_utilityapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Voshell_Michael_UtilityApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.voshell_michael_utilityapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.voshell_michael_utilityapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
